...
#if (DEV_INFRARED)
    #define MD_LEVEL_LOW     7500
    #define MD_LEVEL_MIDDLE  2700
    #define MD_LEVEL_HIGH    1500 //4000
#else
    #define MD_LEVEL_LOW     12000
    #define MD_LEVEL_MIDDLE  7000
    #define MD_LEVEL_HIGH    2500 //4000
#endif
...
******************************************************************************/
static unsigned int g_PIRStateCount = 0;
HI_S32 VDA_MdPrtAp(FILE *fp, VDA_DATA_S *pstVdaData)
{
    ...
    if (g_MotionDetectSensitivity > 0)
    {
        ...
        if (pstVdaData->unData.stMdData.u32AlarmPixCnt > s_MDAlarm_Pix_Cnt)
        {
            ...
            {
                g_PIRStateCount ++;
                printf("................g_PIRStateCount: %d..................\n", g_PIRStateCount);
                if (g_PIRStateCount >= 6)
                {
                    g_PIRStateCount = 0;
                    printf("...PIR Alarming...\n");
...
HI_VOID *VDA_MdGetResult(HI_VOID *pdata)
{
	...
	while (HI_TRUE == pgs_stMdParam->bThreadStart)
	{
		...
		usleep(100*1000); //here must be 100.
	}
