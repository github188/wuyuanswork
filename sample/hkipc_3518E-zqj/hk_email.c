...
static int WriteNetData( int nFd,char *pBuf,int nLen )
{
    int nCntTime = nLen /800;
    int nLeveSize = nLen % 800;
    int nPos = 0;
    while( nCntTime-- )
    {
        //printf( "send %d %d %d \n",nPos,nCntTime,nLen );
        if( send( nFd,pBuf + nPos*800,800,MSG_NOSIGNAL ) <= 0 )
        {
            return -1;
        }
        usleep(1000*100);
    ...
    {
        usleep(500);
        return  send( nFd,pBuf + nPos*800,nLeveSize,MSG_NOSIGNAL );
    }
}

static void CloseSocket( int hksocket )
{
    shutdown( hksocket,SHUT_RDWR );
    close( hksocket );
}
...
static int connect_nonblock(const char *ip,int port,int sec) 
{
    ...
    lint.l_onoff = 1;   
    lint.l_linger = 0;   
    setsockopt( sockfd,SOL_SOCKET,SO_LINGER,(void *)&lint,sizeof(struct linger) );
    */
    int nBufferSize = 65535;
    setsockopt(sockfd,SOL_SOCKET,SO_SNDBUF,(char*)&nBufferSize,sizeof(nBufferSize));
...
static int write_image_buf(int socketfd)
{
	...
	for(i=0; i < hk_email.mcount; i++)
	{
		...
		WriteNetData(socketfd,buffer,strlen(buffer));
		sprintf(buffer,"Content-Type: image/jpeg; name=%s\r\n",imgName);
		if (WriteNetData(socketfd,buffer,strlen(buffer)) < 0)
			return 0;
        ...
        if(WriteNetData(socketfd, (void *)en64_buf, len) <= 0 )
		{
...
{
	if(NULL == smtp_server || NULL == passwd || NULL == send_from || NULL == send_to || NULL == smtp_user)
	{
	...
	unsigned char buffer[2024] = {0},*ptr;
	//unsigned char username[MAXLINE];
	int first_auth = FALSE;
	struct hostent *hent;
	
    if ((strlen(smtp_server)==0) || (strlen(send_from)==0) || (strlen(body)==0) || ( strlen(passwd)==0 ) || (strlen(send_to)==0) || (strlen(smtp_user)==0))
	{
		fprintf(stderr, "smtp_server=%s\n send_from=%s\n body=%s\n passwd=%s\n send_to=%s\n", smtp_server,send_from,body,passwd,send_to);

		return -1;
	}
    printf("passwd=%s.send_from=%s, smtp_user=%s..\n",passwd, send_from, smtp_user);
    ...
    if (WriteNetData(sockfd,buffer,strlen(buffer)) < 0){
        mail_error("EHLO");
        goto error;
    }
    ...
    {
        if ( WriteNetData(sockfd,"AUTH LOGIN\r\n",12)  < 0 )
            goto error;
    ...
    if ( auth_base64_flag != 0  && first_auth == FALSE )
    {
		...
		if (readline2(sockfd, buffer, MAXLINE) < 0)
		{
            HK_DEBUG("22 Check Your Password!\n");
			goto error;
		}

		if ( 0 != strncmp("235 ",buffer,4) ) 
		{
            HK_DEBUG("33 Check Your Password!\n");
			goto error;
		}
		// auth ok!
	}
    ...
    sprintf(buffer,"MAIL FROM: <%s>\r\n",smtp_user );
#endif
    WriteNetData(sockfd,buffer,strlen(buffer));
	readline2(sockfd,buffer,MAXLINE);
