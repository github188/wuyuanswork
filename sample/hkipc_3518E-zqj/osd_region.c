...
int Get_Current_DayTime(unsigned char *pTime)
{
	...
	time(&curTime);
	ptm = gmtime(&curTime);
    if( strcmp(host_, "technaxx.scc21.net")==0 || g_iZone!= 28800 )
    {
        sprintf(tmp, "%02d-%02d-%04d %02d:%02d:%02d", ptm->tm_mday,ptm->tm_mon+1,ptm->tm_year+1900,   ptm->tm_hour, ptm->tm_min, ptm->tm_sec);
    }
    else
    {
...
static unsigned char *g_pASC24FontLib = NULL;

static unsigned char *g_pHZ12FontLib = NULL;
static unsigned char *g_pHZ16FontLib = NULL;
static unsigned char *g_pHZ24FontLib = NULL;
static unsigned char *g_pHZ32FontLib = NULL;
static unsigned char *g_pHZ40FontLib = NULL;
...
inline int GetFontOffset(const unsigned char *pChar,int fontWidth)
{
	...
	else if( ((*pChar) == 0x57)||((*pChar) != 0x4d) )
	{
		...
		case 16:	
			...
			printf("===============>osd 22222222222222222222<===========\n");
			return 0;
			break;
		case 24:
			{
	...
	else if(  ((*pChar) >=0x61) && ((*pChar) <=0x7a) )//小写英文字
	{
		...
		case 16:	
			...
			printf("===============>osd 33333333333333333333333<===========\n");
			return 0;
			break;
		case 24:
			{
	...
	else
	{
		...
		case 16:	
			...
			printf("===============>osd 444444444444444444444444444<===========\n");
			return 0;
			break;
		case 24:
			{
...
int OsdFontRGB1555Bmp(int fontOffset,int fontWidth,int fontHeight,unsigned char *pFont, int bmpOffset,int bmpStride,unsigned char *pBmpBuf)
{
	...
	bG =0x66;//0x82;
	bB= 0x99;//0xFF;
	//bR = bG = bB = 0;//255;//255&0x0c;//黑色//255&0x17;
...
#else
	////加载12X12中文字体	VGA从码流
	/*if(HI_FAILURE == FONT_ReadFile("HZK12",&g_pHZ12FontLib))
	{
		return HI_FAILURE;
	}*/
	////加载16X16中文字体用于720P以上主码流
	if(HI_FAILURE == FONT_ReadFile("HZK16F",&g_pHZ16FontLib))	
	{
		return HI_FAILURE;
	}

	////加载24X24中文字体用于720P以上主码流
	/*if(HI_FAILURE == FONT_ReadFile("HZK24S",&g_pHZ24FontLib))
	{
		return HI_FAILURE;
	}*/
	////加载40X40中文字体用于960P以上主码流
	if(HI_FAILURE == FONT_ReadFile("HZK40T",&g_pHZ40FontLib))
	{
		return HI_FAILURE;
	}
#endif
...
int FONT_Exit()
{
	if(g_pHZ12FontLib) free(g_pHZ12FontLib);
	if(g_pHZ16FontLib) free(g_pHZ16FontLib);	
	if(g_pHZ24FontLib) free(g_pHZ24FontLib);
	if(g_pHZ32FontLib) free(g_pHZ32FontLib);		
	if(g_pHZ40FontLib) free(g_pHZ40FontLib);	
...
HI_S32 VOSD_ShowTextRgn(RGN_HANDLE RgnHandle ,HI_S32 nChn ,char* pStr)
{
	...
	switch(nChn)
	{
		case 0:
			...
			nFontType = FONT_HZ40x40;
			break;
		case 1:
		case 2:
		case 3:
		case 4:
			nFontType = FONT_HZ16x16;//FONT_HZ12x12;//FONT_HZ16x16;			
			//nFontType = FONT_HZ24x24;//FONT_HZ12x12;//FONT_HZ16x16;
			break;
		default:
			break;
	}
	switch(nFontType)
	{
		...
		//case FONT_HZ12x12:
			...
			break;
		case FONT_HZ24x24:		
			pFontLib = g_pHZ24FontLib;
			nFontWidth = 24;
			nFontHeight = 24;			
			break;
		//case FONT_HZ32x32:
		case	FONT_HZ40x40:
			pFontLib = g_pHZ40FontLib;
			nFontWidth = 40;//24;//32;
			nFontHeight = 40;//24;//32;
			break;
		default:
			return HI_FAILURE;
	}
...
static int OSD_Draw_BitMap_EX( int len, const unsigned char *pdata, unsigned char *pbitmap)
{
	...
	{			
		for( i = 0; i < len; i++ )
		{
			...
			for ( w = 0; w < 8; w++ )
			{
				...
				if( (code[2*h]) & flag )
				{
					//*pDst = (0x00<< 10) | (0x00<< 5) | (0x80);//黑色display font  //change font color
					*pDst = (0xff << 10) | (0xff<< 5) | (0xff);//黄色display font
				}
				else
				{
					//*pDst = (0x88<< 10) | (0x88<< 5) | (0x77);//黄色(0xff<< 10) | (0x00<< 5) | (0x00);//background
                    			*pDst = (0x00<< 10) | (0x00<< 5) | (0x0);//黑色
				}
				flag >>= 1;
				xx += 2;
			}
	...
	switch(RgnVencChn)
	{
		case 0://720p
			...
			if(RgnHandle == RgnVencChn)
				...
				stChnAttr.unChnAttr.stOverlayChn.stPoint.s32X = 1280 - (ContentLen*16) - 3*OSD_POSITION_X;
			break;
		case 1://vga
		case 2:
			...
			if(RgnHandle == RgnVencChn)
				...
				stChnAttr.unChnAttr.stOverlayChn.stPoint.s32X = 640 - (ContentLen*16)-8;
			break;
		default:
			stChnAttr.unChnAttr.stOverlayChn.stPoint.s32X = OSD_POSITION_X;
			break;
...
#else
    stChnAttr.unChnAttr.stOverlayChn.u32BgAlpha      =50;// 120;//30;// 50;//48; //[0,128]
#endif
...
int OSD_Overlay_RGN_Display_EX( RGN_HANDLE RgnHandle, const unsigned char *pRgnContent )
{
    HI_S32 s32Ret = HI_FAILURE;
    BITMAP_S stBitmap;
    int ContentLen = 0;
    ...
    /* HI_MPI_RGN_SetBitMap */	
    unsigned char *BitMap = (unsigned char *) malloc(ContentLen*16*24*2); //RGB1555: 2 bytes(R:5 G:5 B:5).
    if (NULL == BitMap)
    {
        SAMPLE_PRT("malloc error with: %d, %s\n", errno, strerror(errno));  
        return HI_FAILURE;
    }
    memset( BitMap, '\0', ContentLen*16*24*2 );

    OSD_Draw_BitMap_EX( ContentLen,pRgnContent, BitMap );

    stBitmap.enPixelFormat 	= PIXEL_FORMAT_RGB_1555;
    stBitmap.u32Width		= 16*ContentLen;
    stBitmap.u32Height 	    = 24;
    stBitmap.pData 			= BitMap;
...
int OSD_Overlay_RGN_Display_DevID( RGN_HANDLE RgnHandle ,VENC_GRP RgnVencChn)
{
    ...
    //strcpy(buf_DevID,"JISIWEI Robot");
    strcpy(buf_DevID,"JISIWEI");
    ...
    OSD_Overlay_RGN_Display_EX( RgnHandle, buf_DevID );
#else
    unsigned char buf_DevID[32] = {0};
...
int OSD_Overlay_RGN_Display_Time( RGN_HANDLE RgnHandle,VENC_GRP RgnVencChn)
{
   ...
   OSD_Overlay_RGN_Display_EX( RgnHandle, buf_Time );
#else
...
int VOSD_Init()
{
    ...
    //strcpy(buf_RGN,"JISIWEI Robot");
    strcpy(buf_RGN,"JISIWEI");
...
HI_S32 VOSD_Init()
{
	...
	for(nChn=0;nChn<RgnVencChn;nChn++)
	{
		switch(nChn)
		{
			case 0:
				fontWidth = 40;//24;//32;
				Rect[nChn].u32Width = 40*40;// 24*24;//32*32;
				Rect[nChn].u32Height= 40;//24;//32;
				break;
			case 1:
			case 2:
			case 3:
			...
			#if 1
				fontWidth = 16;
				Rect[nChn].u32Width = 16*16;//16*16;
				Rect[nChn].u32Height= 16;//16;
			#else
				fontWidth = 24;
				Rect[nChn].u32Width = 24*24;//16*16;
				Rect[nChn].u32Height= 24;//16;
			#endif
				break;
			default:
				break;
		...
		switch(nChn)
		{
			case 0:
				...
				Rect[nChn].s32X = 1280 - (TimeBufLen*32) + OSD_POSITION_X;
				break;
			case 1:
			case 2:
			case 4:
				Rect[nChn].s32Y = OSD_POSITION_Y-8-8;
				//Rect[nChn].s32X = 640 - (TimeBufLen*16)+8*OSD_POSITION_X;				
				Rect[nChn].s32X = 640 - (TimeBufLen*16)+2*OSD_POSITION_X;
				break;
			case 3:
				Rect[nChn].s32Y = OSD_POSITION_Y-12;
