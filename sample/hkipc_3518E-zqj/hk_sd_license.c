 ...
 *    id=E12345678;Psw=abcdef;mac1=00:14:04:18:20:19;pichost=www.uipcam.com;nettype=0;ptztype=1;
 *    id=E87654321;Psw=fedcba;mac1=00:14:04:18:20:19;pichost=www.uipcam.com;
 **************************************************************************************************************************/
...
static int Get_SD_License_Info(License_Info_st *pLicense)
{
    ...
    sscanf(line_buf, "%*[^=]=%[^;]%*[^=]=%[^;]%*[^=]=%[^;]%*[^=]=%[^;]%*[^=]=%[^;]%*[^=]=%[^;]%*[^=]=%[^;]", 
            pLicense->CamID, pLicense->CamPwd, pLicense->CamMac1, pLicense->CamProxy, pLicense->CamNettype, pLicense->CamPtztype, pLicense->CamM433Enable);
...
int HK_Check_SD_License(int SdOnline)
{
    ...
    if (0 == access(CONF_SD_LICENSE, F_OK | R_OK)) //check file existence and readalbe.
    {
        ...
        if (0 == strlen(devUser))
        {
            ...
            printf("[%s, %d] CamID:%s, CamPwd:%s, CamMac1:%s, CamProxy:%s, CamNetType:%s, CamPtzType:%s, CamM433:%s\n", \
                    __func__, __LINE__, SD_LicenseInfo.CamID, SD_LicenseInfo.CamPwd, SD_LicenseInfo.CamMac1,            \
                    SD_LicenseInfo.CamProxy, SD_LicenseInfo.CamNettype, SD_LicenseInfo.CamPtztype, SD_LicenseInfo.CamM433Enable);
