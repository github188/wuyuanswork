...
int Get_PTZ_PresetParams(int nValue, int *xPos, int *yPos)
{
	...
	int iPos = conf_get_int(PTZ_CONF, presetXName);		
    if( iPos ==0 )
        *xPos=1;
    else
        *xPos = iPos;
	int iPoy = conf_get_int(PTZ_CONF, presetYName);
    if(iPoy == 0 )
        *yPos = 1;
    else
        *yPos = iPoy;
...
{		
    ...
    if ((g_stPtzPreset[nPos].presetX <= 1) || (g_stPtzPreset[nPos].presetX > PTZ_RANGE_LR))
    {
    ...
    if ((g_stPtzPreset[nPos].presetY <= 1) || (g_stPtzPreset[nPos].presetY > PTZ_RANGE_UD))
    {
